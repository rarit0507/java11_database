...
<table style="width:1400px;margin:30px auto" class="tb">
	<thead>
		<tr>
			...
			<th>bytes</th><th>blocks</th><th>create_bytes</th>
	...
	<c:forEach var="data" items="${dList }" varStatus="status">
		<tr>
			<td>${data.creation_time }</td><td>${data.status }</td>
			<td>${data.enabled }</td><td>${data.bytes }</td>
			<td>${data.blocks }</td><td>${data.create_bytes }</td>
			<td>${data.block_size }</td><td>${data.name }</td>
		</tr>
	</c:forEach>	
	</tbody>
</table>
...
<table style="width:1200px;margin:30px auto" class="tb">
	<thead>
		<tr>
			<th>name</th><th>is_recovery_dest_file</th>
	...
	<c:forEach var="data" items="${cList }" varStatus="status">
		<tr>
			<td>${data.name }</td><td>${data.is_recovery_dest_file }</td>
			<td>${data.block_size }</td><td>${data.file_size_blks }</td>
			<td>${data.con_id }</td>
		</tr>
	</c:forEach>	
	</tbody>
...
<table style="width:1200px;margin:30px auto" class="tb">
	<thead>
		<tr>
			...
			<th>is_recovery_dest_file</th><th>con_id</th>
		</tr>
	</thead>
	<tbody>
	<c:forEach var="data" items="${lList }" varStatus="status">
		<tr>
			<td>${data.type }</td><td>${data.member }</td>
